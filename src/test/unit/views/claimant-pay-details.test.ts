...
const pageJson = JSON.parse(
  fs.readFileSync(
    path.resolve(__dirname, '../../../main/resources/locales/en/translation/claimant-pay-details.json'),
    'utf-8'
  )
);
...
describe('Claimant pay details page', () => {
  beforeAll(async () => {
  ...
  it('should display subtitle', () => {
    const subtitle = htmlRes.getElementsByClassName(subtitleClass);
    expect(subtitle[2].innerHTML).contains(expectedSubtitle, 'Page title does not exist');
  });
