import RespondToNotificationCopyController from '../../../main/controllers/RespondToNotificationCopyController';
...
describe('Respond to Notification Copy to Other Party Controller', () => {
  let controller: RespondToNotificationCopyController;
  let request: ReturnType<typeof mockRequest>;
  let response: ReturnType<typeof mockResponse>;

  beforeEach(() => {
    controller = new RespondToNotificationCopyController();
    request = mockRequest({});
  ...
  describe('POST method', () => {
    ...
    it('should render the same page when copyToOtherPartyYesOrNo empty', async () => {
      ...
      await controller.post(request, response);
      expect(response.redirect).toHaveBeenCalledWith(
        PageUrls.RESPOND_TO_NOTIFICATION_COPY + languages.ENGLISH_URL_PARAMETER
      );
