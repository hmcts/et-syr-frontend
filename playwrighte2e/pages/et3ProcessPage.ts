...
export default class ET3ProcessPage extends BasePage {
  async submitET3Response(): Promise<void> {
    ...
    await this.webActions.verifyElementContainsText(
      this.page.locator('legend span'),
      'Did we receive the ET3 response in time?',
      3000
    );
    await this.clickContinue();
    await this.delay(2000);
    ...
    await this.webActions.verifyElementContainsText(
      this.page.locator('legend span'),
      "Do we have the respondent's name?",
      3000
    );
    await this.clickContinue();
    await this.delay(2000);
    ...
    await this.webActions.verifyElementContainsText(
      this.page.locator('#confirmation-header markdown h1'),
      'ET3 Processing complete',
      10000
    );
